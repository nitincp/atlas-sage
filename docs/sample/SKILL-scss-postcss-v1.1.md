...
### Fix 1 — BEM child element extraction
Add to parser: extract nested BEM elements (`&__element` and `&__element--modifier`) as separate nodes.
These carry domain state meaning that must not be buried inside the parent style-rule chunk.

```javascript
// Inside the style-rule walk, add:
node.walk(child => {
  if (child.type === 'rule' && child.selector.startsWith('&__')) {
    const bemEdges = [];
    const bemSel = child.selector.replace('&', node.selector);
    bemEdges.push({ type: 'BELONGS_TO', target_node_id: node.selector.trim(), confidence: 'deterministic' });
    nodes.push({
      node_id: uuidv4(),
      language: 'scss',
      source_file: filePath,
      chunk_type: 'bem-element',
      raw_cleaned: child.toString(),
      edges: bemEdges
    });
  }
});
...
New edge type: **BELONGS_TO** — deterministic — BEM element belongs to parent component.

### Fix 2 — Infrastructure rules
Add to summarisation instructions for style-rule chunk:
- If selector is `*`, `body`, `html`, or contains `::before`/`::after` global resets → classify as `infrastructure-rule`, summary: "Global CSS reset / base typography. Not a UI component."
- Do not attempt domain inference on infrastructure rules.

### Fix 3 — Contextual summarisation for truncate-text usage
Add to summarisation instructions for style-rule chunk:
- If a rule INCLUDES `truncate-text` — note which element is being truncated and why it matters
- Truncated text implies variable-length data from the backend. Note this as a data shape signal.
- Example: `__price @include truncate-text` → "Price display truncates long values — implies variable-length pricing data, possibly multi-currency or dynamic fare structures"

### Updated Known Limitations
- Dynamic class names computed in JavaScript remain unresolvable
- STYLES edge remains probabilistic
- Pseudo-selectors (:hover, :disabled) not extracted — interaction states not captured
