...
6. Reason about cross-file structural relationships. Edge types to consider:
   ...
   - INJECTS: constructor/initialiser in file A declares a parameter typed as class/interface B from another file (deterministic)
7. For each relationship, call store_edge with:
   ...
   - confidence: "deterministic" for import/inherit/implement/inject, "probabilistic" for calls, "inferred" if uncertain
   - evidence: one sentence explaining the structural evidence (e.g. "OrderService.__init__ takes IOrderRepository parameter")
